...
class Api {
    ...
    async getUser(params) {
        ...
        let keys = Array.from(sp.keys());
        for (let key of keys) {
            if ((sp.get(key) === "undefined") || (sp.get(key) === "null")) sp.delete(key);
        }
        let response;
        try {
            response = await axios.get(`${this.base}/users/${user}/${mode || ''}`, {
                params: sp
            });
        }
        catch (ex) {
            if (ex?.response?.statusText) throw "连接失败，" + ex.response.statusText;
            throw "连接失败";
        }
        if (!response || !response?.data) throw "获取数据失败，可能查无此人";
    ...
    async getBest(params) {
        ...
        let response;
        try {
            response = await axios.get(`${this.base}/best/${user}/${mode || ''}`, {
                params: sp
            });
        }
        catch (ex) {
        ...
        if (!response || !response?.data) throw "获取数据失败";
        if (response?.data?.error) throw response?.error?.message;
        // if (response?.data?.messages?.length > 0) throw response?.messages?.join("\n");
    ...
    async getRecentScore(params) {
        ...
        let response;
        try {
            response = await axios.get(`${this.base}/recent/${user}/${mode || ''}`, {
                params: sp
            });
        }
        catch (ex) {
    ...
    async getScore(params) {
        ...
        let response;
        try {
            response = await axios.get(`${this.base}/scores/${mode}/${id}`, {
                params: sp
            });
        }
        catch (ex) {
